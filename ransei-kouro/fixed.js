...
function draw(){bd.innerHTML='';a.forEach((p,i)=>{let e=document.createElement('div');e.className='cell '+(sel===i?'pick':'')+(sel>=0&&ok(i)?' can':'');e.onclick=()=>moveTo(i);if(p){let u=document.createElement('div'),t=p.toUpperCase(),r=rows[p===t?t:t.toLowerCase()];u.className='unit '+(p===t?'':'blue');u.style.backgroundImage=`url(${p===t?'./shogi-player.png':'./shogi-enemy.png'})`;u.style.backgroundSize='100% 1100%';u.style.backgroundPosition=`0 ${r/10*100}%`;e.append(u)}bd.append(e)});tr.innerHTML='';h.forEach((p,i)=>{let b=document.createElement('button');b.className='buy';b.textContent=p;b.onclick=()=>{drop=i;sel=-1;g.textContent='Choose an empty square.';draw()};tr.append(b)});st.textContent=side?'Computer turn':'Your turn'}
